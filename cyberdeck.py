...
parser.add_argument("-s", "--speed", default=2, help="Set the speed of the scan. 1 is the slowest, 4 is the fastest. Default is 2.", type=int)
...
if len(sys.argv) > 1:
    ...
    if args.scan:
        ...
        if args.target is None and (args.synscan or args.udpscan or args.tcpscan or args.osdetect):
        ...
        elif args.speed not in [1, 2, 3, 4]:
            parser.error("Speed must be an integer between 1 and 4.")
            ...
            if args.synscan:
                scan.parsing_scanoptions("syn", destination_ip=args.target, scan_quietness=args.speed, targeted_ports=args.portrange, showClosed=args.showclosed)
            if args.udpscan:
                scan.parsing_scanoptions("udp", destination_ip=args.target, targeted_ports=args.portrange, udp_specificity=args.istargeted, scan_quietness=args.speed, showClosed=args.showclosed)
            if args.tcpscan:
                scan.parsing_scanoptions("tcp", destination_ip=args.target, scan_quietness=args.speed, targeted_ports=args.portrange, showClosed=args.showclosed)
            if args.osdetect:
    ...
    print("\n Choose your quickhack:\n\nh. Help\nq. Quit\n1. Scanning\n2. \n3. Author") #as more functionalities will be added, more options will be available here
