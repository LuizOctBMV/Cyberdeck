...
def syn_incomplete_connection(port, destination_ip, source_ip, quietness): 
    port_n_state = [port, "n/a"]

    lst_syn_packlet = create_syn_packet(port, destination_ip, source_ip)

    raw_socket = socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_TCP)
    raw_socket.settimeout(quietness[0])
...
def syn_scan(ports_to_be_scanned, destination_ip, quietness, scan_display, source_ip):

    sic_w_IP = partial(syn_incomplete_connection, destination_ip=destination_ip, quietness=quietness, source_ip=source_ip) # allows the IP address and other parameters to be included on the threading process
    with ThreadPoolExecutor(max_workers=quietness[1]) as executor:
        results = executor.map(sic_w_IP, ports_to_be_scanned)

    printing_results(results, scan_display)

def tcp_complete_connection(port, destination_ip, quietness): 
...
def tcp_scan(ports_to_be_scanned, destination_ip, scan_quietness, scan_display): 

    tcc_w_IP = partial(tcp_complete_connection, destination_ip=destination_ip, quietness=scan_quietness) # allows the IP address and other parameters to be included on the threading process
    with ThreadPoolExecutor(max_workers=scan_quietness[1]) as executor:
            results = executor.map(tcc_w_IP, ports_to_be_scanned)
    
    printing_results(results, scan_display)

def printing_results(results, scan_display): 
    ...
    if scan_display == 1:
        ...
        print(f"{amountTimedOut} ports timed out")
...
def portscanning():
    ...
    if scan_type == 3:
        ...
        if scan_type == 1: 
            tcp_scan(ports_to_be_scanned, destination_ip, quietness, scan_display)
        else: 
            source_ip = get_source_ip(destination_ip)
            syn_scan(ports_to_be_scanned, destination_ip, quietness, scan_display, source_ip)
